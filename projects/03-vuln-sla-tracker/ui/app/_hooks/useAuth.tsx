...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    try {
      ...
      localStorage.setItem("itgc_token", data.access_token);
      localStorage.setItem("itgc_user", JSON.stringify(data.user));
      setToken(data.access_token);
      setUser(data.user);
      return {};
    } catch { return { error: "Network error. Is the backend running?" }; }
  }, []);
  ...
  const logout = useCallback(() => {
    localStorage.removeItem("itgc_token"); localStorage.removeItem("itgc_user");
